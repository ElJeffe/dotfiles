...
COMPILER_NAME = ["cc", "gcc", "g++", "cpp", "c++", "gcc3", "ccache", "ccdist", "if ppc_440-gcc", "if" ]
IGNORE_LINES = ["then", "rm", "ar", "Making", "mv"]
...
colordict = { 
      ...
      "dcopidl":     "[1;33m",
      "dcopidl2cpp":   "[1;33m",
      "normal":     "[0;30m" # disable bold when finishing
...
prefdict = { 
      ...
      "dcopidl":     "",
      "dcopidl2cpp": "",
      "normal":     "" # disable bold when finishing
      #"normal":     "[1;37;40m" # enable bold when finishing
      }
def processLine( words ):
  ...
  #  #
  #un#known = hidden + warnother + unknown;
  #formatOutput( unknown, "unknown", 1)
...
def outputline( line, colorcode ):
  global USE_COLOR, colordict, prefdict

  if len( line ) == 0 : return
  if USE_COLOR: sys.stdout.write(colordict[ colorcode ])
  else: sys.stdout.write(prefdict[ colorcode ])
  print line,
...
#  print opts, args
  ...
  try:
    ...
    while len(l) :
      ...
      if first_word == "sh":
        ...
        if make1 != "":
          outputline("Building " + make1, "building")
          make1 = ""
        if make2 != "":
          outputline("Building " + make2, "building")
          make2 = ""
        compiled = compiled + 1
        processLine( words )
      elif first_word == "ppc_440-ranlib":
        if make1 != "":
          outputline("Building " + make1, "building")
          make1 = ""
        if make2 != "":
        ...
        libbed = libbed + 1
        unknown = []
        unknown.append("Creating")
        unknown.append(words[1])
        formatOutput( unknown, "unknown", 1)
      elif first_word == "ppc_440-g++":
        if make1 != "":
          outputline("Building " + make1, "building")
          make1 = ""
        if make2 != "":
        ...
        formatOutput( unknown, "unknown", 1)
      elif first_word.startswith("make[1"):
        if words[1] == "Entering":
          make1 = words[3].split("/")[-1].rstrip("\'")
        elif words[1] == "Leaving":
          make1 = ""
      elif first_word.startswith("make[2"):
        if words[1] == "Entering":
          make2 = words[3].split("/")[-1].rstrip("\'")
        elif words[1] == "Leaving":
          make2 = ""
      elif first_word.startswith("collect2"):
        if l.find("returned 1"):
          outputline("Linking failed.", "unknown")
          errors = errors + 1
      elif first_word in IGNORE_LINES or first_word.startswith("make[") or first_word.startswith("ppc_440-ar"):
        i = 1
      else:
        if l.find("error:") != -1:
          errors = errors + 1
        elif l.find("No such file or directory") != -1: # include not found
          errors = errors + 1
  ...
  summary = ""
  if compiled:
    summary += "Compiled %d file" % compiled
    if compiled > 1:
      summary += "s"
    if libbed or linked:
      summary += " - "
  if libbed:
    summary += "Created %d lib" % libbed
    if libbed > 1:
      summary += "s"
    if linked:
      summary += " - "
  if linked:
    summary += "Linked %d program" % linked
    if linked > 1:
      summary += "s"
  if summary == "":
    print "Nothing done."
  else:
    outputline(summary, "unknown")
    outputline("%d errors, %d warnings" % (errors, warnings), "unknown")
  timer = time.time() - timer
  outputline("Total time %02d:%02d" %(timer/60, timer%60), "unknown")
  if proc.poll() != 0:
    sys.exit(1)
